...
import { getPinnedSessionIds } from "./pinned-sessions";
...
async function loadAllSessions(): Promise<SessionInfo[]> {
	...
	const projectByCwd = new Map<string, ProjectInfo>(
		await Promise.all(
			uniqueCwds.map(
				async (cwd): Promise<[string, ProjectInfo]> => [
					cwd,
	...
	return piSessions.map((s) => {
		...
		return {
			...
			parentSessionId: s.parentSessionPath
				? pathToId.get(normalizePath(s.parentSessionPath))
				: undefined,
			projectRoot: project?.projectRoot ?? s.cwd,
			...(project?.isWorktree && project.branch
				? { worktreeBranch: project.branch }
				: {}),
			...(pinnedIds.has(s.id) ? { pinned: true } : {}),
		};
	});
}
...
export function cacheSessionPath(sessionId: string, filePath: string): void {
	...
	if (
		...
		reverseCache.get(previousPath) === sessionId
	) {
	...
	if (
		...
		pathCache.get(previousSessionId) === pathKey
	) {
	...
	try {
		...
		while (position < maxHeaderBytes && !foundNewline) {
			const buffer = Buffer.allocUnsafe(
				Math.min(4096, maxHeaderBytes - position),
			);
...
function entryToUiMessage(
	...
	// The early return below passes non-assistant messages through unchanged.
